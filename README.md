...
- **Least privilege.** dbt connects via a dedicated `transformer` role/`dbt_user` in Snowflake, scoped to `SELECT` on raw and full DML on dev/prod schemas — not the account admin role.
